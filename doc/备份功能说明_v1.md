...
### 运行依赖
- 新增依赖：
  - `archiver@^6.0.1`（用于 zip 打包）
  - `check-disk-space@^3.0.4`（用于检测磁盘剩余空间）
- 启动/安装：
```bash
npm install
npm start
...
#### 磁盘空间校验
- 备份前会估算源数据大小（近似），并检测当前运行盘的可用空间。
- 若可用空间低于阈值，则返回 `507 Insufficient Storage`，并包含：
  - `freeBytes`：当前可用空间
  - `requiredFreeBytes`：所需最小可用空间（阈值）
  - `sourceBytes`：源数据估算大小
- 阈值策略：`max(BACKUP_MIN_FREE_BYTES, floor(sourceBytes * 0.1))`
  - 环境变量 `BACKUP_MIN_FREE_BYTES` 默认为 `50MB`
  - 说明：虽然备份以流式下载为主，不落地临时文件，但给出保守阈值以降低系统临时空间不足的风险。
...
### 变更摘要
- 后端：
  - 新增 `archiver` 与 `check-disk-space` 依赖。
  - 增加 `GET /api/admin/backup` 路由；计算源目录估算大小，进行磁盘空间校验（不足时返回 507），设置 `X-Source-Bytes`，流式输出 zip。
- 前端：
